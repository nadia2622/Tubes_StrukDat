...
#include "include/Login.h"        // Include class Login
#include "include/Auth.h"         // Include class Auth
#include "src/Login.cpp"          // Include implementasi Login
#include "src/Auth.cpp"           // Include implementasi Auth
...
void adminMainMenu(BST &tree, KomikManager &manager, Auth &auth);  // Menu untuk Admin
void userMainMenu(BST &tree, KomikManager &manager, Auth &auth);   // Menu untuk User
void crudKomikMenu(BST &tree, KomikManager &manager);
void viewKomikMenu(BST &tree, int option, KomikManager &manager);  // View only untuk User
...
int main()
{
    ...
    while (running)
    {
        int choice = login.showLoginMenu();
        ...
        case 1:
        {
            ...
            if (user != nullptr)
            {
                ...
                cout << "\n\033[32mLogin successful! Welcome, " << user->username << "!\033[0m\n";
                ...
                if (auth.isAdmin())
                {
                    adminMainMenu(tree, manager, auth);
                }
                else
                {
                    userMainMenu(tree, manager, auth);
                }

                // Setelah keluar dari menu, logout
                auth.logout();
            }
            else
            {
                // Login gagal
                cout << "\n\033[31mLogin failed! Invalid username or password.\033[0m\n";
                pause();
            }
            break;
        }
        ...
        case 2:
        {
            // ===== REGISTER =====
            string username, password;
            login.showRegisterForm(username, password);

            // Register user baru
            if (auth.registerUser(username, password))
            {
                cout << "\n\033[32mYou can now login with your new account.\033[0m\n";
            }
            pause();
            break;
        }
